...
const HEARTBEAT_MS = 20_000; // latido real cada 20s: si paran, el icono vuelve a gris
const LIVESTREAM_WAIT_MS = 35_000;
const LIVESTREAM_RETRY_AFTER_MS = 12_000;
...
const heartbeatTimers = new Map(); // latidos por gafa
...
// Latido cada 20s: confirma que las gafas siguen vivas de verdad.
// Si las apagas, este latido deja de llegar y el icono se pone gris solo.
function startHeartbeat(session, deviceId) {
  ...
  heartbeatTimers.set(deviceId, id);
}
function stopHeartbeat(deviceId) {
  const id = heartbeatTimers.get(deviceId);
  if (id) clearInterval(id);
...
class CaosServer extends ServerBase {
  async onSession(session, sessionId, userId) {
    ...
    session.events?.onDisconnected?.(async () => {
      ...
      await sendToCaos({ device_id: deviceId, event_type: "session_close" });
